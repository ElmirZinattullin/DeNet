...
async def write_to_disk(user: User, file: UploadFile, media_path:str, storage: Storage, address: int) -> str:
    # with open(f'{MEDIA_PATH}/user_{user.id}/storage_{storage.id}', "rb") as file:
    #     file.write(data)
    path = Path(f"user_{user.id}/storage_{storage.id}")
    absolute_path = Path(media_path).absolute() / path
    ...
    async with aiofiles.open(media_path / file_path, mode="wb") as f:
